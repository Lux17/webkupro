...
class LaporanhasilController extends Controller
{
    public function index(Request $request)
    {
        ...
        $id_user =  DB::table('hasil_diagnosis')->where('id_hasil', $kode)->pluck('idpengguna');
        $find_user = DB::table('users')->where('id', $id_user)->get();

        $hasil_diagnosis = DB::table('hasil_diagnosis')->where('id_hasil', $kode)->paginate();
        ...
        for ($i = $cari_gejala1 ; $i < $cari_gejala2; $i++) {
            $kondisiUser[] = [
        ...
        // dd($combined);
        return view('laporan_hasil', ['hasil_diagnosis' => $hasil_diagnosis,'penyakit' => $penyakit,'top3' => $top3,'gejala' => $gejala,'find_user' => $find_user,'gejala2' => $gejala2,'analisa' => $analisa,'combined' => $combined,'cari_gejala' => $cari_gejala,'kondisiArray2' => $kondisiArray2, 'kondisiUser' => $kondisiUser, 'lihat_user' => $lihat_user]);
